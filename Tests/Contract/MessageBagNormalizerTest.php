...
final class MessageBagNormalizerTest extends TestCase
{
    ...
    /**
     * @param array{contents: list<array{role: 'model'|'user', parts: array<int, mixed>}>, system_instruction?: array{parts: array{text: string}[]}} $expected
     */
    ...
    public static function provideMessageBagData(): iterable
    {
        ...
        yield 'text with image' => [
            new MessageBag(
                Message::ofUser('Tell me about this instrument', Image::fromFile(\dirname(__DIR__).'/Fixtures/image.jpg'))
            ),
            [
                'contents' => [
                    ['role' => 'user', 'parts' => [
                        ['text' => 'Tell me about this instrument'],
                        ['inline_data' => ['mime_type' => 'image/jpeg', 'data' => base64_encode(file_get_contents(\dirname(__DIR__).'/Fixtures/image.jpg'))]],
                    ]],
                ],
            ],
        ];
